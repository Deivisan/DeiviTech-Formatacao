...
// 🎯 Configuração
const GROK_LINK = process.argv[2] || 'https://grok.com/share/c2hhcmQtMg_f3757fed-9fe4-471b-9142-592d8b432fa5';
...
async function extractConversation(page) {
  ...
  await page.waitForTimeout(5000); // Aguardar 5s extra
  ...
  const messages = await page.evaluate(() => {
    ...
    if (elements.length === 0) {
      console.log('Nenhum seletor funcionou, usando fallback de texto bruto');
      const bodyText = document.body.innerText;
      // Tentar dividir por padrões de mensagens
      const lines = bodyText.split('\n').filter(line => line.trim().length > 10);
      return lines.map((line, idx) => ({
        index: idx,
        author: line.includes('Deivison') ? 'Deivison' : 'Grok',
        content: line.trim(),
        timestamp: null,
        type: 'fallback'
      }));
    ...
    elements.forEach((el, idx) => {
      ...
      const authorEl = el.querySelector('[data-author]') || 
                       ...
                       el.querySelector('b');
      let author = authorEl?.innerText?.trim() || 'unknown';
      
      // Se não encontrou, tentar padrões no texto
      if (author === 'unknown') {
        if (text.includes('Deivison') || text.match(/\b(você|eu|meu|minha)\b/i)) {
          author = 'Deivison';
        } else {
          author = 'Grok';
        }
...
(async () => {
  ...
  await page.goto(GROK_LINK, { waitUntil: 'load', timeout: 120000 }); // Aumentado timeout e mudado para 'load'
